/**
 * Filters domain module for the Thunderbird MCP server.
 *
 * Implements the message-filter tools: listFilters, createFilter,
 * updateFilter, deleteFilter, reorderFilters, applyFilters, plus their
 * internal helpers (getFilterListForAccount, serializeFilter, buildTerms,
 * buildActions) and the Thunderbird filter constant maps (ATTRIB_MAP,
 * OP_MAP, ACTION_MAP and their reverse lookups).
 *
 * Loaded by api.js via Services.scriptloader.loadSubScript with a
 * { module: { exports: {} } } scope, the same mechanism used for
 * security_helpers.js. register(ctx) destructures the shared dependencies
 * (XPCOM bindings plus the infra helpers for account access and the
 * forward/reply filter gate) from ctx, defines the constants and functions
 * verbatim, and assigns the tool functions back onto ctx for the dispatch
 * switch.
 *
 * Function bodies are copied byte-for-byte from the monolithic api.js so
