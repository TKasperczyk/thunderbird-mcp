...
// Auto-generate the tool-registry manifest before packaging. Scans
// extension/mcp_server/lib/tools/**/*.sys.mjs (skipping files prefixed
// with _), sorts the paths, writes _manifest.sys.mjs. Consumed at
// extension-load time by lib/tool-registry.sys.mjs. See that module's
...
(function generateToolManifest() {
  ...
  (function walk(dir, rel) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      const full = path.join(dir, entry.name);
      const relNext = rel + entry.name;
      if (entry.isDirectory()) {
        walk(full, relNext + '/');
      } else if (entry.name.endsWith('.sys.mjs')) {
        files.push(relNext);
      }
    }
  })(toolsDir, '');
  files.sort();
  const manifestPath = path.join(toolsDir, '_manifest.sys.mjs');
  const body =
    ...
    'export const TOOL_MODULES = ' + JSON.stringify(files, null, 2) + ';\n';
  fs.writeFileSync(manifestPath, body);
  console.log(`Tool manifest: ${files.length} tool(s) registered (${manifestPath})`);
})();
